...
from data_generation import causal_generation, random_select_bootstrap
from data_generation import causal_generation_bootstrap
from confidence_interval import IC
from slearner import SLearner

       
def visualization(beta, bias, d, f, g, p, base_metalearner = SLearner()):
    ...
    for i in range(100, 2000, 100):
        Nsamples = 250
        sample_size = int(0.8*i)
        X, W, Y =  causal_generation(i, d, beta, bias, f, g, p)
        samples = random_select_bootstrap(i, sample_size, Nsamples)
        mu, inf, sup = IC(X, W, Y, samples, base_metalearner, alpha=0.05)
        ates.append(mu)
        ates_inf.append(inf)
        ates_sup.append(sup)
    ...
    plt.ylabel("Valeur ATE")
    plt.title("Evaluation de l'ATE en fonction du nombre d'observations")
    plt.legend(loc=1,numpoints=1)
    
    plt.savefig('simulation_vis.pdf')
    plt.show();
    
    
    
def graphic_comparison(nb_obs, d, p, beta, bias, f, g, base_learner_homemade, base_learner_causalml):
  ...
  for n in nb_obs :
    ...
    samples = random_select_bootstrap(n, sample_size, Nsamples)
    mu, inf, sup = IC(X, W, Y, samples, base_learner_homemade, alpha=0.05)
